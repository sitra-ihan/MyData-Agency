...
exports.anonCrypt = async function(did, message) {
  let verkey = await sdk.keyForDid(
    await indy.pool.get(),
...
exports.anonDecrypt = async function(did, message) {
  ...
  let decryptedMessageBuffer = await sdk.cryptoAnonDecrypt(
    await indy.wallet.get(),
    verKey,
    message
  );
...
exports.sendAnonCryptedMessage = async function(did, message) {
  var targetDID = undefined;
  let isConnReq = await indy.connections.isConnectionRequest(message.type);
  if (isConnReq === true) {
    targetDID = did;
  } else {
    targetDID = await exports.resolvePublicDIDtoPairwiseDID(did);
  }
  message = JSON.stringify(message);
  let endpoint = await indy.pool.getEndpointForDid(did);
  let encryptedMessage = await exports.anonCrypt(did, message);
  return indy.messages.sendMessage(endpoint, encryptedMessage, targetDID);
};

exports.resolvePublicDIDtoPairwiseDID = async function(publicDID) {
  let connections = await indy.pairwise.getAll();
  for (let connection of connections) {
    if (connection.metadata.theirEndpointDid == publicDID) {
...
exports.authCrypt = async function(myDid, theirDid, message) {
  ...
  let buffer = await sdk.cryptoAuthCrypt(
    await indy.wallet.get(),
    myVerkey,
    theirVerkey,
...
exports.authDecrypt = async function(myDid, message) {
  let myVerkey = await sdk.keyForLocalDid(await indy.wallet.get(), myDid);
  let [, decryptedMessageBuffer] = await sdk.cryptoAuthDecrypt(
    await indy.wallet.get(),
    myVerkey,
...
exports.buildAuthcryptedMessage = async function(
  myDid,
  theirDid,
  messageType,
  message
...
exports.getVerKey = async function(did) {
  let verkey = await sdk.keyForDid(
    await indy.pool.get(),
    await indy.wallet.get(),
    did
