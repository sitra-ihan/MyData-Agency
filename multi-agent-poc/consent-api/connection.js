...
module.exports = {
  send_connection_request: async function send_connection_request(did, token) {
    try {
      let key = rsa.RSAKeyBuffer();
      let decodedToken = authService.verifyToken(token, key.public);
      ...
      let response = await indy.crypto.sendAnonCryptedMessage(
        theirEndpointDid,
        connectionRequest
      );
      return {
  ...
  get_their_relationship_did: async function get_their_relationship_did(
    their_public_did
  ) {
    try {
  ...
  get_my_relationship_did: async function get_my_relationship_did(
    their_public_did
  ) {
    try {
      let pairwise_list = await indy.pairwise.getAll();
