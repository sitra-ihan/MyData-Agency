...
exports.pendingAccessRequests = {
  getAll: function() {
    init();
    return store.pendingAccessRequests;
  },
  write: function(senderDid, accessRequest) {
    init();
    let id = uuid();
    store.pendingAccessRequests.push({
      id: id,
      theirEndpointDid: senderDid,
  ...
  get: function(id) {
    init();
    for (let message of store.pendingAccessRequests) {
      if (message.id === id) {
        return message;
      }
    }
  ...
  delete: function(id) {
    init();
    for (let i = 0; i < store.pendingAccessRequests.length; i++) {
      if (store.pendingAccessRequests[i].id === id) {
        store.pendingAccessRequests.splice(i, 1);
      }
    }
    syncChanges();
  }
